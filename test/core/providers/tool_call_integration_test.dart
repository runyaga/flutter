...
void main() {
  group('Tool call integration', () {
    ...
    test(
      'secret-number happy path — process, execute, synthesize, map',
      () async {
        ...
        expect(
          conversation.toolCalls[0].status,
          equals(ToolCallStatus.pending),
        );
        expect(
          conversation.toolCalls[1].status,
          equals(ToolCallStatus.pending),
        );
    ...
    group('FakeAgUiClient', () {
      test('onRunAgent returns different streams per call', () async {
        ...
        final events1 = await client
            .runAgent('agentic_chat', const SimpleRunAgentInput(threadId: 't1'))
            .toList();
      ...
      test('default stream is empty when onRunAgent is null', () async {
        ...
        final events = await client
            .runAgent('agentic_chat', const SimpleRunAgentInput())
            .toList();
