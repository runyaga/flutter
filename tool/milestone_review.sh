...
if [[ $# -lt 1 ]]; then
  ...
  echo "    --skip-all         Skip both tests and analyze"
...
TEST_OUTPUT=$(mktemp)
trap 'rm -f "$METRICS_AFTER" "$ANALYZE_OUTPUT" "$TEST_OUTPUT"' EXIT
...
# -------------------------------------------------------
# Phase 7: Assemble prompt markdown
# -------------------------------------------------------
echo "=== Phase 7: Assembling prompt ==="
...
1. **This file** — review instructions, milestone spec, metrics
...
echo "      (2) analyze each of the 8 rubric items with PASS/CONCERN/FAIL and"
