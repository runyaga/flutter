...
2. **Drop List Check**: Are there any files from the DROP LIST that leaked in?
...
3. **Import Hygiene**: Are all imports correct? No circular deps? No stale imports?
4. **Test Coverage**: Are tests present and correct for changed code?
5. **Minimal Change**: Is the change minimal — no over-engineering or scope creep?
